...
category: "fbcover",
usages: "facebook cover photo make by SK Siddik",
cooldowns: 0,
dependencies: {
    "fs-extra": "",
    "request": "",
...
if ((this.config.credits) != "SK-SIDDIK-KHAN") { return api.sendMessage(`[ WARNING ] - SK-SIDDIK-KHAN Don't credits modules ${this.config.name} was changed to ${this.config.credits} bởi ADMINBOT ${global.config.BOTNAME} 😐 Stop now!!!`, event.threadID, event.messageID)}
    const request = require('request');
  const fs = require("fs-extra")
  const axios = require("axios")
  const { threadID, messageID, senderID, body } = event;
  if(!args[0]) return api.sendMessage('♡︎ Hey Itz Me Sk Siddik ♡︎\n\n➡️Please enter primary name', threadID, messageID)
  else return api.sendMessage(`➡️You chose the main name : ${args.join(" ").toUpperCase()}\n\n(Reply to this message and choose your secondary name)`,event.threadID, (err, info) => {
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  switch (handleReply.type) {
    ...
    case "sdt": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`➡️You have selected phone nombar : ${event.body.toUpperCase()}\n\n(Reply this message to enter your email)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "email",
          name: `fbcover`,
          author: senderID,
    ...
    case "email": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`➡️You have selected email as : ${event.body.toUpperCase()}\n\n(Reply this message to enter your address)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "color",
          name: `fbcover`,
          author: senderID,
    ...
    case "create": {
      ...
      api.unsendMessage(handleReply.messageID);
      api.sendMessage(`Places Wait⏳`,threadID, (err, info) => {
      setTimeout(() => {
      ...
      if(!fs.existsSync(__dirname+`/cache/UTMAvoBold.ttf`)) { 
          let getfont2 = (await axios.get(`https://drive.google.com/u/0/uc?id=1E2wOdqkC_fpe9SIVlodMOh9cY3Cduj-p&export=download`, { responseType: "arraybuffer" })).data;
