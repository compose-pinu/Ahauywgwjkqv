...
module.exports.onLoad = async () => {
  ...
  const url = "https://drive.google.com/uc?id=1C7LshVJ_J-NLIpKPis7LiuMg0Nf9jnEI";
...
module.exports.run = async function ({ event, api, args, Currencies }) { 
  ...
  if (!userCurrency) {
    ...
    userCurrency = await Currencies.getData(senderID); 
  }
  ...
  if (!two) {
    ...
    return makeImage({ one, two }).then(path => {
      api.sendMessage({ 
        body: `${ae[Math.floor(Math.random() * ae.length)]}\n Horimism to you after being kissed is ${hc} %\n + ${((hc) * rd)} $`, 
